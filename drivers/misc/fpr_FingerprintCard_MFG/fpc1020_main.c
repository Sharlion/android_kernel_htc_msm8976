...
/* -------------------------------------------------------------------- */
#ifdef CONFIG_FBx
static void fpc1020_early_suspend(struct device *dev);

static void fpc1020_late_resume(struct device *dev);
...
/* HTC fingerprint attribute start*/
int fp_sensor = 0;
fpc1020_data_t *fpc1020_temp = NULL;
		...
		const char *buf, size_t count)
{
	...
	if(enable != pre_enable)
	{
		if(enable)
		{
			...
			pre_enable = enable;
			printk("[fp][FPC]Enable l22 regulator!\n");
		}
		else
		{
			retval = regulator_disable(regulator_l22);
			if (retval) {
				printk("[fp][FPC]Fail to disable l22 regulator\n");
				return retval;
			}
			pre_enable = enable;
			printk("[fp][FPC]Disable l22 regulator!\n");
		}

	}
	else
		printk("[fp][FPC]enable == pre_enable(%d)\n", enable);
		...
		const char *buf, size_t count)
{
	...
	sscanf(buf, "%d", &deep_sleep);
	deep_sleep = deep_sleep % 2;
	if(deep_sleep)
		fpc1020_sleep(fpc1020_temp, true);
	else
		fpc1020_sleep(fpc1020_temp, false);
...
static DEVICE_ATTR(power, S_IRUGO | S_IWUSR , NULL, power_store);
static DEVICE_ATTR(sleep, S_IRUGO | S_IWUSR , NULL, sleep_store);
...
static int fingerprint_sysfs_init(void)
{
        ...
        ret = sysfs_create_file(android_fingerprint_kobj, &dev_attr_power.attr);
        if (ret) {
                printk("%s: sysfs_create_file read_att failed (power)\n", __func__);
                return ret;
        }
        ret = sysfs_create_file(android_fingerprint_kobj, &dev_attr_sleep.attr);
        if (ret) {
                printk("%s: sysfs_create_file read_att failed (sleep)\n", __func__);
                return ret;
        }
	/*
        ret = sysfs_create_file(android_fingerprint_kobj, &dev_attr_clk.attr);
        if (ret) {
                printk("%s: sysfs_create_file read_att failed(clk)\n", __func__);
                return ret;
        }
        ret = sysfs_create_file(android_fingerprint_kobj, &dev_attr_fp_wakeup.attr);
        if (ret) {
                printk("%s: sysfs_create_file read_att failed(fp_wakeup)\n", __func__);
                return ret;
        }
        printk("attribute file register Done");

        ret = sysfs_create_file(android_fingerprint_kobj, &dev_attr_fp_mode.attr);
        if (ret) {
                printk("%s: sysfs_create_file read_att failed(fp_mode)\n", __func__);
                return ret;
        }
        printk("attribute file fp_mode register Done");
	*/
        return 0;
}

/* HTC fingerprint attribute End*/
static int __init fpc1020_init(void)
{
	printk(KERN_INFO "%s\n", __func__);
...
static int fpc1020_power_init(fpc1020_data_t *fpc1020,struct device *dev)
{
	...
	msm_spi_pinctrl_set(0);


	regulator_l22 = regulator_get(dev, "l22");
...
#else
		fingerprint_pinctrl = devm_pinctrl_get(dev);
		state = pinctrl_lookup_state(fingerprint_pinctrl, "fpc1020_reset_reset");
		if (IS_ERR(state)) {
			dev_err(dev, "cannot find '%s'\n", "fpc1020_reset_reset");
		}
		retval = pinctrl_select_state(fingerprint_pinctrl, state);
		if (retval)
...
*/
	...
	printk(KERN_INFO "%s MFG version\n", __func__);
	...
	error = fpc1020_power_init(fpc1020, dev);
	if (error)
		goto err;
...
*/
	...
	fingerprint_sysfs_init(); //fingerprint atrribute create
...
static void fpc1020_early_suspend(struct device *dev)
{
	...
	printk("%s\n", __func__);
	fpc1020_worker_goto_idle(fpc1020_temp);
	fpc1020_wake_up(fpc1020_temp);
	fpc1020_write_sensor_setup(fpc1020_temp);
	fpc1020_sleep(fpc1020_temp, false);
	if(0)
        {
                printk("[fp][CT]Disable interrupt before suspend\n");
                disable_irq_nosync(fpc1020_temp->irq);
                irq_set_irq_wake(fpc1020_temp->irq, 0);
        }
...
static void fpc1020_late_resume(struct device *dev)
{
	...
	if(0)
        {
                printk("[fp][CT]Enable interrupt after resume\n");
                enable_irq(fpc1020_temp->irq);
                irq_set_irq_wake(fpc1020_temp->irq, 1);
        }
}
#endif
...
static int fb_notifier_callback(struct notifier_block *self,
					 unsigned long event, void *data)
{
	//struct fpc1020_data_t *fpc1020 = container_of(self, struct fpc1020_data_t, fb_notifier);
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK) {
		...
		case FB_BLANK_UNBLANK:
			fpc1020_late_resume(fpc1020_temp->device);
			break;
		case FB_BLANK_POWERDOWN:
			fpc1020_early_suspend(fpc1020_temp->device);
			break;
		}
	}
					...
					struct fpc1020_platform_data *pdata)
{
	...
	struct device_node *fpc_node = dev->of_node;
#if 0
	/* required properties */
	const void *irq_prop = of_get_named_gpio(node, "fpc_gpio_irq",   NULL);
	const void *rst_prop = of_get_named_gpio(node, "fpc_gpio_reset", NULL);
	const void *cs_prop  = of_get_named_gpio(node, "fpc_gpio_cs",    NULL);
#endif
...
#else
	pdata->irq_gpio   = of_get_named_gpio(fpc_node, "fpc_gpio_irq",   0);
	pdata->reset_gpio = of_get_named_gpio(fpc_node, "fpc_gpio_reset", 0);
	pdata->cs_gpio    = of_get_named_gpio(fpc_node, "fpc_gpio_cs",    0);
#endif
