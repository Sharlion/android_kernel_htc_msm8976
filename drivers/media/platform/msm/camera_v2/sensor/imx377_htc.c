...
#define EEPROM_DEFECT_PIXEL_SHIFT_X -26 
#define EEPROM_DEFECT_PIXEL_SHIFT_Y -18 
    ...
    if (first)
    {
        ...
        for (address = EEPROM_DEFECT_PIXEL_ADDR; address <= EEPROM_DEFECT_PIXEL_ADDR + MAX_DEFECT_PIXELS*4; address += 4)
        {
            ...
            if (dx != 0 || dy != 0)
            {
                ...
                defect_pixels.px[defect_pixels.count++].y = y + EEPROM_DEFECT_PIXEL_SHIFT_Y + ((dy & 0x04) ? -(dy & 0x3) : (dy & 0x3) ); 
            }
        }
