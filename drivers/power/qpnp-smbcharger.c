...
#define TEMP_THR_WARM_H		450
#define TEMP_THR_WARM_L		430
static void
smbchg_adjust_fastchg_current(int vbat_mv)
{
	...
	static int s_batt_health_warm_thr = TEMP_THR_WARM_H;
#endif

#ifdef CONFIG_MACH_PMEC2
	if (batt_temp >= TEMP_THR_WARM_H) {
		s_batt_health_warm_thr  = TEMP_THR_WARM_L;
		is_warm = true;
	} else if ((TEMP_THR_WARM_H > batt_temp) && (batt_temp > TEMP_THR_WARM_L)) {
		
		if (s_batt_health_warm_thr == TEMP_THR_WARM_H)
			is_warm = false;
		
		if (s_batt_health_warm_thr == TEMP_THR_WARM_L)
			is_warm = true;
	} else { 
		s_batt_health_warm_thr = TEMP_THR_WARM_H;
		is_warm = false;
	}
