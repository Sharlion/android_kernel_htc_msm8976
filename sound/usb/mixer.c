			 ...
			 unsigned char *name, int maxlen, int term_only)
{
	...
	if (iterm->type >> 16) {
		...
		case UAC_SELECTOR_UNIT:
			strlcpy(name, "Selector", sizeof(name)-1); return 8; 
		case UAC1_PROCESSING_UNIT:
			strlcpy(name, "Process Unit", sizeof(name)-1); return 12; 
		case UAC1_EXTENSION_UNIT:
			strlcpy(name, "Ext Unit", sizeof(name)-1); return 8; 
		case UAC_MIXER_UNIT:
			strlcpy(name, "Mixer", sizeof(name)-1); return 5; 
		default:
			return snprintf(name, sizeof(name)-1, "Unit %d", iterm->id); 
		}
	}

	switch (iterm->type & 0xff00) {
	case 0x0100:
		strlcpy(name, "PCM", sizeof(name)-1); return 3; 
	case 0x0200:
		strlcpy(name, "Mic", sizeof(name)-1); return 3; 
	case 0x0400:
		strlcpy(name, "Headset", sizeof(name)-1); return 7; 
	case 0x0500:
		strlcpy(name, "Phone", sizeof(name)-1); return 5; 
	}

	for (names = iterm_names; names->type; names++)
		if (names->type == iterm->type) {
			strlcpy(name, names->name, sizeof(name)-1); 
			      ...
			      int readonly_mask)
{
	...
	case UAC_FU_VOLUME:
		if (! len) {
			...
			if (! len)
				len = snprintf(kctl->id.name, sizeof(kctl->id.name)-1, 
					       "Feature %d", unitid);
		}
...
#define MAX_ITEM_NAME_LEN	64
	for (i = 0; i < desc->bNrInPins; i++) {
		...
		if (! len)
			snprintf(namelist[i], sizeof(namelist[i])-1, "Input %d", i); 
	}
			 ...
			 int ignore_error)
{
	...
	strlcpy(chip->card->mixername, "USB Mixer", sizeof(chip->card->mixername)-1); 
